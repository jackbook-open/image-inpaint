...
| --- | --- | --- |
| Windows users can download and start without Python or the command line. | `ImageInpaint-Windows-x64.zip`, `ImageInpaint-Windows-x64.zip.sha256`, full-extract smoke, installed launch smoke, and manual non-developer Windows user smoke within five minutes. | `packaging/build-windows.ps1 -RuntimeDir ...`, `packaging/verify-windows-zip-smoke.ps1 -FullExtract`, manual evidence. |
